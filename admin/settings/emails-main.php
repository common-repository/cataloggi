...
// subs extensible 
$subs = CTLGGI_Admin::ctlggi_admin_settings_emails_subs();
...
{  
	...
	if( isset($_GET['tab']) && $_GET['tab'] == "emails-main")
	{
		...
		$baseurl = home_url() . '/wp-admin/edit.php?post_type=cataloggi&page=cataloggi-settings&tab=emails-main&sub=';
		...
		// page content
		if ( file_exists( CTLGGI_PLUGIN_DIR . 'admin/settings/emails/' . $pageslug . '.php' ) ) {
		  require $pagerequire = plugin_dir_path( dirname( __FILE__ ) ) . 'settings/emails/' . $pageslug . '.php';
		} else {
		  // make it extensible
		  do_action( 'ctlggi_admin_add_settings_emails_sub_page' ); // <- extensible
