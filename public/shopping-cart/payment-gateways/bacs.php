...
class CTLGGI_Gateway_Bacs {
	 ...
	 * @return void
	 */
    public static function ctlggi_order_data_for_gateway_bacs( $order_data ) 
	{
	 ...
	 * @return object
	 */
    public function ctlggi_process_bacs_payment() 
	{	
		...
		if ( $validate_order_form == 'ok' ) {
			...
			if ( isset( $order_data['form_type'] ) && $order_data['form_type'] == 'paymentsform' ) {
				$this->ctlggi_bacs_payments_form_process( $orderdata );
			} elseif ( isset( $order_data['form_type'] ) && $order_data['form_type'] == 'checkoutform' ) {
				$this->ctlggi_bacs_checkout_form_process( $orderdata );
			} else {
				return;
			}
	 ...
	 * @return void
	 */
    public function ctlggi_bacs_payments_form_process( $orderdata ) 
	{	
		if ( empty( $orderdata ) )
		return;
		...
		if ( ! empty($order_id) ) {
			...
			if ( isset( $order_data['order_status'] ) && $order_data['order_status'] == 'completed' ) {
				...
				update_post_meta( $order_id, '_order_transaction_id', $transaction_id_bacs );
	 ...
	 * @return void
	 */
    public function ctlggi_bacs_checkout_form_process( $orderdata ) 
	{	
		if ( empty( $orderdata ) )
		return;
